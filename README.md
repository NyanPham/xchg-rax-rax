...
.loop:
    ...
    loop     .loop
```
Compute the Fibonacci sequence up to the n<sup>th</sup> element, where n is stored in the `ecx` register.   
...
This resembles the implementation of an adder in electronics. The `xor` operation is to find the output of a bit, and the `and` operation is to detect a carry for more siginificant bit position.
...
If `eax` is negative, the `add` operation overflows the 33th bit of `rax` (the lowest bit in upper half), causing all bits in the upper half to be cleared. Then `xor` operation restores the upper half of `rdx` to be negative.

If `eax` is positive, the overflow never occurs after the `add` operation. Then `xor` to remove the upper half of `rdx`, leaving `rax` positive.
...
xor      rdx,rcx
```
Performs `xor` operation of the two consecutive Gray Codes.
The `shr rdx, 1` instruction and `xor` the original number results in gray code of that number.
...
or       eax,ecx
```
Swaps a certain amount of bits. This illustrates the [Bit-reversal permutation](https://en.wikipedia.org/wiki/Bit-reversal_permutation).  
...
The snippet, decreases the value by 1, copies the most set significant bits to the least significant ones (all bits are set to one). Finally, increment the number to get the power of two. 
...
xor      eax,edx
```
Computes the next state in [LFSR](https://en.wikipedia.org/wiki/Linear-feedback_shift_register).   
